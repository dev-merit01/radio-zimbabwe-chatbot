...
Working branch: `fix/voting-pipeline-reliability`.
...
- Full Python suite: **111 passed, 2 skipped**, with 4 delivery-response subtests
...
listener; the existing CI workflow will execute it against PostgreSQL.

## Release limits and follow-up

These are source changes only; no production data, provider messages, published
charts or live deployment were changed. No GitHub branch has been pushed yet.

1. Run the existing PostgreSQL CI job on the final commit before release. This
   environment has no PostgreSQL server, and package installation was blocked by
   its process/user restrictions. SQLite cannot verify row-lock concurrency.
...
3. Browser journeys and native Windows installation were not rerun in this
   backend-focused change. Keep the existing browser/installer CI gates.
...
Existing deployment instructions remain in `docs/WINDOWS_V2.md`. Update application
