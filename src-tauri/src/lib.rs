...
pub fn load_configuration(path: &Path) -> Result<Option<Configuration>, String> {
    let data = match fs::read(path) {
        ...
        Err(_) => {
            return Err("Saved settings could not be read. Enter your server address again.".into())
        }
    };
...
pub fn save_configuration(path: &Path, server: &Url) -> Result<(), String> {
    ...
    file.as_file()
        ...
        .map_err(|_| "Could not save the server address.")?;
    file.persist(path)
        .map_err(|_| "Could not replace the saved settings.")?;
    Ok(())
}
...
mod tests {
    ...
    fn accepts_https_origins_and_normalizes_them() {
        assert_eq!(
            validate_server(" https://STATION.example:443 ")
                .unwrap()
                .as_str(),
    ...
    fn rejects_unsafe_or_non_origin_addresses() {
        for value in [
            ...
            "https://station.example/?secret=x",
    ...
    fn navigation_is_restricted_to_the_configured_origin() {
        ...
        assert!(allowed_navigation(
            &server,
            &Url::parse("https://station.example/accounts/login/?next=/").unwrap()
        ));
        for value in [
            "https://other.example/",
            "https://station.example:444/",
            "http://station.example/",
            "https://tauri.localhost/",
            "file:///etc/passwd",
        ] {
    ...
    fn settings_survive_replacement_and_store_only_the_server() {
        ...
        for address in ["https://first.example", "https://second.example"] {
            ...
            assert_eq!(
                load_configuration(&path).unwrap().unwrap().server_url,
                server.as_str()
            );
        }
