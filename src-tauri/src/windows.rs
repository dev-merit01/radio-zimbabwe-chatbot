...
use serde::Serialize;
use tauri::{menu::{MenuBuilder, SubmenuBuilder}, webview::{DownloadEvent, NewWindowResponse},
...
fn config_path(app: &AppHandle) -> Result<PathBuf, String> {
    app.path().app_config_dir().map(|path| path.join("config.json"))
...
fn require_setup(window: &WebviewWindow) -> Result<(), String> {
    ...
    let url = window.url().map_err(|_| "Connection settings are unavailable.")?;
    if window.label() != "setup" || url.scheme() != "https" || url.host_str() != Some("tauri.localhost") {
...
fn get_configuration(app: AppHandle, window: WebviewWindow) -> Result<SetupConfiguration, String> {
    ...
    let fallback = option_env!("VOTING_STUDIO_SERVER_URL").unwrap_or("").to_string();
    match load_configuration(&config_path(&app)?) {
        Ok(Some(config)) => Ok(SetupConfiguration { server_url: config.server_url, warning: String::new() }),
        Ok(None) => Ok(SetupConfiguration { server_url: fallback, warning: String::new() }),
        Err(warning) => Ok(SetupConfiguration { server_url: fallback, warning }),
    }
}

#[tauri::command]
async fn connect_server(app: AppHandle, window: WebviewWindow, state: tauri::State<'_, Connecting>, server_url: String) -> Result<(), String> {
    require_setup(&window)?;
    if state.inner().0.compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire).is_err() {
    ...
    let client = reqwest::Client::builder()
        ...
        .build().map_err(|_| "Could not prepare a secure connection.")?;
    let mut response = client.get(server.join("api/desktop/status").map_err(|_| "Invalid server address.")?)
        .send().await.map_err(|_| "Cannot connect. Check the server address and network, then try again.")?;
    if !response.status().is_success() {
        return Err("The server is unavailable or needs the Voting Studio desktop update.".into());
    }
    let mut bytes = Vec::new();
    while let Some(chunk) = response.chunk().await.map_err(|_| "The connection was interrupted. Try again.")? {
    ...
    if let Some(old) = app.get_webview_window("workspace") {
        old.destroy().map_err(|_| "Close the existing workspace and try again.")?;
    }
    ...
    workspace.set_focus().map_err(|_| "Could not focus the workspace.")?;
    window.hide().map_err(|_| "Could not close the connection screen.")?;
    Ok(())
}
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            let connection = SubmenuBuilder::new(app, "Application")
                ...
                .separator().text("quit", "Exit").build()?;
            let edit = SubmenuBuilder::new(app, "Edit").cut().copy().paste().select_all().build()?;
            app.set_menu(MenuBuilder::new(app).item(&connection).item(&edit).build()?)?;
            Ok(())
        })
        .on_menu_event(|app, event| match event.id().as_ref() {
            "connection" => show_setup(app),
            "reload" => { if let Some(window) = app.get_webview_window("workspace") { let _ = window.reload(); } },
            "quit" => app.exit(0),
            _ => {},
        })
