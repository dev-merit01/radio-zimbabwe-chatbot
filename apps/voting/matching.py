...
def find_existing_song_match(
    artist_normalized: str, song_normalized: str, date
) -> Optional[RawSongTally]:
    """
    ...
    for tally in existing_tallies:
        ...
        artist_similar = is_similar(
            artist_normalized, existing_artist, ARTIST_SIMILARITY_THRESHOLD
        )
        if not artist_similar:
            continue

        # Check if song is similar
        song_similar = is_similar(
            song_normalized, existing_song, SONG_SIMILARITY_THRESHOLD
        )
        if song_similar:
            return tally
...
def smart_normalize_vote(
    artist_raw: str, song_raw: str, vote_date
    ...
    artist_display, song_display, match_key, display_name = normalize_vote_input(
        artist_raw, song_raw
    )
...
def match_against_known_songs(
    artist_input: str, song_input: str, threshold: float = AUTO_MERGE_THRESHOLD
    ...
    for song in verified_songs:
        ...
        if (
            artist_token_overlap >= MIN_TOKEN_OVERLAP
            and title_token_overlap >= MIN_TOKEN_OVERLAP
        ):
    ...
    if best_score >= threshold and (best_score - second_best_score) >= CONFIDENCE_GAP:
        match_key = (
            f"{normalize_text(best_song.artist)}::{normalize_text(best_song.title)}"
        )
        return (best_song.artist, best_song.title, match_key, best_song.canonical_name)
...
def merge_song_tallies(source_tally: RawSongTally, target_tally: RawSongTally):
    ...
    with transaction.atomic():
        ...
        source_votes = RawVote.objects.filter(
            match_key=source_tally.match_key, vote_date=source_tally.date
        )

        # Check for users who already voted for target
        target_user_ids = set(
            RawVote.objects.filter(
                match_key=target_tally.match_key, vote_date=target_tally.date
        ...
        remaining_votes.update(
            match_key=target_tally.match_key, display_name=target_tally.display_name
        )
