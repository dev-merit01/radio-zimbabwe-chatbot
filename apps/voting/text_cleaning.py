...
def extract_featured_artists(text: str) -> Tuple[str, List[str]]:
    ...
    if feat_match:
        ...
        featured = [
            a.strip() for a in re.split(r",\s*|\s+and\s+", featured_part) if a.strip()
        ]
...
def clean_vote_text(artist_raw: str, song_raw: str) -> Tuple[str, str]:
    ...
    if featured_str:
        ...
        corrected_featured = [
            correct_artist_typo(name.strip()) for name in featured_names
        ]
    ...
    if song_featured:
        ...
        all_featured.extend(
            [correct_artist_typo(a.strip()) for a in song_featured_names if a.strip()]
        )
        if all_featured:
