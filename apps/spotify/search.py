...
HIGH_CONFIDENCE = (
    0.80  # Auto-accept (lowered from 0.85 for better local music matching)
)
...
def _get_client():
    ...
    if _client is None:
        ...
        _client = spotipy.Spotify(
            auth_manager=auth, requests_timeout=10, retries=0, status_retries=0
        )
    return _client
...
def _search_spotify(
    query: str, limit: int = 5, market: str = SPOTIFY_MARKET
) -> List[dict]:
...
def resolve_with_confidence(artist: str, title: str) -> Tuple[Optional[dict], float]:
    ...
    try:
        results = _search_spotify(
            f"track:{search_title} artist:{search_artist}", limit=5
        )
        all_results.extend(results)
    ...
    try:
        results = _search_spotify(
            f"{search_artist} {search_title}", limit=5, market=None
        )
        all_results.extend(results)
    except SpotifyLookupError:
        pass
    ...
    for result in unique_results:
        ...
        if _is_zimbabwean_artist(result["artists"]):
            ...
            logger.debug(
                "Zimbabwean artist boost applied for: %s", ", ".join(result["artists"])
            )
