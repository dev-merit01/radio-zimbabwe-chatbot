...
class BirdClient:
    ...
    def _post(self, endpoint: str, payload: dict) -> dict:
        ...
        try:
            response = self._session.post(url, json=payload, timeout=(5, 20))
            response.raise_for_status()
        except requests.RequestException as exc:
            logger.error("Provider request failed (%s)", type(exc).__name__)
            raise BirdClientError("Provider request failed") from exc
    ...
    def send_text(self, phone_number: str, text: str) -> dict:
        ...
        endpoint = (
            f"/workspaces/{self.workspace_id}/channels/{self.channel_id}/messages"
        )

        payload = {
            "receiver": {"contacts": [{"identifierValue": phone_number}]},
    ...
    def send_image(self, phone_number: str, image_url: str, caption: str = "") -> dict:
        ...
        payload = {
            ...
            "body": {
                "type": "image",
                "image": {"mediaUrl": image_url, "caption": caption},
            },
        }
...
def _resolve_credentials() -> tuple[str, str, str]:
    ...
    access_key = getattr(settings, "BIRD_ACCESS_KEY", "") or os.environ.get(
        "BIRD_ACCESS_KEY", ""
    )
    workspace_id = getattr(settings, "BIRD_WORKSPACE_ID", "") or os.environ.get(
        "BIRD_WORKSPACE_ID", ""
    )
    channel_id = getattr(settings, "BIRD_CHANNEL_ID", "") or os.environ.get(
        "BIRD_CHANNEL_ID", ""
    )
...
def get_client() -> BirdClient:
    ...
    with _client_lock:
        if _client_instance is None or (
            _client_instance.access_key,
            _client_instance.workspace_id,
            _client_instance.channel_id,
        ) != (access_key, workspace_id, channel_id):
...
def send_text_async(phone_number: str, text: str) -> None:
    ...
    def _send():
        try:
            ...
            logger.error(
                "Failed to send Bird WhatsApp message to %s: %s", phone_number, exc
            )
