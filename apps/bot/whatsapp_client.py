...
class WhatsAppClient:
    ...
    def __init__(
        self, app_key: str, auth_key: str, session: Optional[requests.Session] = None
    ):
        if not app_key or not auth_key:
            raise WhatsAppConfigurationError(
                "OneMsg app_key and auth_key are required."
            )
    ...
    def _post(self, endpoint: str, payload: dict) -> dict:
        ...
        try:
            response = self._session.post(url, data=payload, timeout=(5, 15))
            response.raise_for_status()
        except requests.RequestException as exc:
            logger.error("Provider request failed (%s)", type(exc).__name__)
            raise WhatsAppClientError("Provider request failed") from exc
...
def _resolve_credentials() -> tuple[str, str]:
    ...
    app_key = getattr(settings, "ONEMSG_APP_KEY", "") or os.environ.get(
        "ONEMSG_APP_KEY", ""
    )
    auth_key = getattr(settings, "ONEMSG_AUTH_KEY", "") or os.environ.get(
        "ONEMSG_AUTH_KEY", ""
    )

    if not app_key or not auth_key:
        raise WhatsAppConfigurationError(
            "OneMsg credentials are not configured. Set ONEMSG_APP_KEY and ONEMSG_AUTH_KEY."
        )
...
def get_client() -> WhatsAppClient:
    ...
    with _client_lock:
        if _client_instance is None or (
            _client_instance.app_key,
            _client_instance.auth_key,
        ) != (app_key, auth_key):
